...
- **Arena Tarung** (`/tarung`) — mini-game battle vs bot: pilih avatar trainer lewat **carousel looping** (kartu tengah selalu default aktif, panah kiri/kanan tampil di mobile & desktop, klik kartu untuk "mencabutnya" baru tombol Pilih muncul) → isi nickname → pilih **Mode Battle** (1 lawan 1 instan) atau **Mode Challenge** (susun tim 3 Pokemon, masuk **Lobi Challenge** dengan level tracker **15 level** trainer berjenjang sampai boss Mewtwo/Arceus). Tiap kali menang/kalah 1 level, muncul **layar hasil duel 2 avatar trainer** (kamu vs lawan, satu ditandai MENANG satu KALAH) dengan animasi confetti (menang) atau kelopak bunga gugur (kalah) serta musik yang menyesuaikan, sebelum lanjut ke lobi manual lewat tombol. Level yang sudah dikalahkan bisa diulang untuk grinding. Tiap kelipatan 3x menang muncul **gacha 4-tier** (persentase diatur admin lewat `/admin/gacha`, default 10% legendaris / 20% evolusi tahap 2 / 30% non-evolusi / 40% bonus evolusi — evolusi Pokemon di tim sendiri, sekarang jadi bagian gacha, bukan tombol terpisah lagi) — hasilnya bisa dipilih untuk menggantikan/mengevolusikan anggota tim atau dibatalkan, progres level **tidak reset**. Skill punya **cooldown 1 giliran** setelah dipakai. Efek suara lengkap (serang, kena hit, pilih trainer, mulai battle, Pokemon sendiri/lawan pingsan, gacha biasa/legendaris, menang, kalah — 10 slot total) **bisa di-custom lewat upload** di `/admin/sound`, plus tiap trainer bisa punya suara "pilih" sendiri di form editnya (fallback ke default global kalau kosong). Kalau tidak diisi, semua otomatis pakai suara sintesis bawaan (Web Audio API, tanpa file eksternal). Gacha tier **Legendaris** dapat animasi spesial (rays emas berputar, partikel sparkle, glow pulsing, shine sweep) biar terasa lebih mewah, dan stat-nya otomatis dinaikkan proporsional kalau total base stat aslinya di bawah 650 (khusus Pokemon "mitos" seperti Shaymin/Celebi/Jirachi yang di game aslinya memang lebih lemah dari legendaris trio/box art) — supaya tiap hasil gacha Legendaris tetap berasa kuat, tanpa mengubah data di katalog. Setiap kemenangan **non-gacha** juga dapat **bonus stat acak** (+3 sampai +7 di 2 stat berbeda, dipilih secara acak berbobot yang cenderung menaikkan Pokemon dengan total stat paling rendah di tim biar makin balance), ditampilkan di layar khusus sebelum lanjut ke lobi. Dibangun dengan React + Inertia (public, tanpa perlu login).
